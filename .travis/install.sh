...
docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive; apt-get -qq install -y   python-pytango'
if [ $? -ne "0" ]
then
    exit -1
fi
...
docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq install -y  nxsconfigserver-db; sleep 10; apt-get -qq install -y   python-nxsconfigserver python-nxstools pyqt4-dev-tools python-qt4'
if [ $? -ne "0" ]
then
    exit -1
fi
